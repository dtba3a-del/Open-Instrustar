...
int main( int argc, char *argv[] ) {
    ...
    // [MOD] Собственная идентичность проекта. Побочный эффект намеренный:
    // настройки хранятся по новому пути, поэтому унаследованные от
    // OpenHantek6022 залипшие значения (в т.ч. те, из-за которых
    // пропадал выбор AC-связи) не подхватываются — приложение
    // стартует с чистого состояния.
    QCoreApplication::setOrganizationName( "Ctpu-bintape-timechannel" );
    QCoreApplication::setOrganizationDomain( "ctpu.local" );
    QCoreApplication::setApplicationName( "Ctpu-bintape-timechannel" );
    ...
    { // do this early at program start ...
        ...
        if ( !doNotTranslate && useLocale && QLocale().name().left( 3 ) != "en_" ) { // l18n only in these cases
            ...
            if ( parserTranslator.load( QLocale(), QLatin1String( "ctpu" ), QLatin1String( "_" ),
    ...
    if ( !doNotTranslate && useLocale &&
        ...
        if ( openHantekTranslator.load( QLocale(), QLatin1String( "ctpu" ), QLatin1String( "_" ),
                                        QLatin1String( ":/translations" ) ) ) {
    ...
    if ( verboseLevel < 2 )
        std::cerr << "CtPU-BinTape-TimeChannel "; // 1st part
    ...
    if ( verboseLevel >= 2 )
        std::cerr << "CtPU-BinTape-TimeChannel has stopped after "; // part 1..4
