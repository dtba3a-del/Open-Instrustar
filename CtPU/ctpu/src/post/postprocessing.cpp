...
void PostProcessing::convertData( const DSOsamples *source, PPresult *destination ) {
    ...
    // [MOD] TZ §2.4.2 — propagate per-channel physical unit strings.
    // The legacy code wrote only the MATH slot's `mathVoltageUnit` scalar;
    // the new pipeline carries a per-channel `physicalUnits` vector that
    // covers physical + math channels. We also keep `voltageUnit` (the
    // legacy enum) in sync for code that still reads it.
    for ( ChannelID channel = 0; channel < source->data.size(); ++channel ) {
        DataChannel *const channelData = destination->modifiableData( channel );
        QString unit = ( channel < source->physicalUnits.size() ) ? source->physicalUnits[ channel ] : QStringLiteral( "V" );
        if ( unit.isEmpty() )
            unit = QStringLiteral( "V" );
        channelData->physicalUnit = unit;
        // Backward-compat: derive the legacy Unit enum from the string.
        if ( unit == QStringLiteral( "V" ) || unit.isEmpty() )
            channelData->voltageUnit = UNIT_VOLTS;
        else if ( unit == QStringLiteral( "V²" ) || unit == QStringLiteral( "W" ) )
            channelData->voltageUnit = ( unit == QStringLiteral( "W" ) ) ? UNIT_WATTS : UNIT_VOLTSQUARE;
        else
            channelData->voltageUnit = UNIT_NONE;
    }
    destination->tag = source->tag;
    destination->captureTimestampMs = source->captureTimestampMs;
}
