...
void SpectrumGenerator::process( PPresult *result ) {
    ...
    for ( ChannelID channel = 0; channel < result->channelCount(); ++channel ) {
        ...
        // --- CCtPU measurement set (v1, confirmed scope in chat) ---
        // Top/Base: mean of samples in the outer 10% band of the displayed
        // [min,max] window (same [left,right] window used for vmin/vmax
        // above) - robust against a narrow ringing/overshoot spike, unlike
        // bare vmax/vmin. Amplitude = top-base is intentionally NOT the same
        // as Vpp = vmax-vmin (Vpp includes overshoot, amplitude doesn't).
        {
            const double range = max - min;
            double topSum = 0.0, baseSum = 0.0;
            int topCount = 0, baseCount = 0;
            const double topThresh = max - 0.10 * range;
            const double baseThresh = min + 0.10 * range;
            for ( int position = left; position <= right; ++position ) {
                double s = channelData->voltage.samples[ unsigned( position ) ];
                if ( s >= topThresh ) {
                    topSum += s;
                    ++topCount;
                }
            ...
            channelData->top = topCount ? topSum / topCount : max;
            channelData->base = baseCount ? baseSum / baseCount : min;
            channelData->amplitude = channelData->top - channelData->base;
        }

        // Per-cycle DC/std-dev and per-edge Overshoot+/Overshoot-: only
        // meaningful when actually triggered. The 2nd/3rd edge sample index
        // is reconstructed from triggeredPosition + pulseWidth1/2 (durations
        // already computed by Triggering::searchTriggeredPosition(), which
        // already does noise-robust averaged level-crossing detection) -
        // no need to duplicate edge-search logic here.
        channelData->dcCycle = dc;
        channelData->stdDevCycle = channelData->ac;
        channelData->overshootRise = 0.0;
        channelData->overshootFall = 0.0;
        if ( result->triggeredPosition > 0 && result->pulseWidth1 > 0.0 && result->pulseWidth2 > 0.0 &&
             channelData->voltage.interval > 0.0 ) {
            const double sr = 1.0 / channelData->voltage.interval;
            const int edge1 = int( result->triggeredPosition );
            const int edge2 = edge1 + int( std::lround( result->pulseWidth1 * sr ) );
            const int edge3 = edge2 + int( std::lround( result->pulseWidth2 * sr ) );
            if ( edge1 >= 0 && edge3 > edge1 && edge3 <= sampleCount ) {
                double cDc = 0.0;
                for ( int i = edge1; i < edge3; ++i )
                    cDc += channelData->voltage.samples[ unsigned( i ) ];
                const int n = edge3 - edge1;
                cDc /= double( n );
                double cAc2 = 0.0;
                for ( int i = edge1; i < edge3; ++i ) {
                    double d = channelData->voltage.samples[ unsigned( i ) ] - cDc;
                    cAc2 += d * d;
                }
                cAc2 /= double( n );
                channelData->dcCycle = cDc;
                channelData->stdDevCycle = sqrt( cAc2 );

                // Per-edge overshoot: short window just after each slope
                // crossing (10% of that edge's own half-period, at least 3
                // samples), classified rising/falling by comparing the mean
                // just-before vs just-after the edge.
                auto edgeOvershoot = [&]( int edgeIdx, int halfPeriodSamples ) {
                    const int win = std::max( 3, int( 0.10 * halfPeriodSamples ) );
                    const int lo = std::max( edgeIdx - win, left );
                    const int hiAfter = std::min( edgeIdx + win, right );
                    if ( edgeIdx <= lo || hiAfter <= edgeIdx )
                        return;
                    double before = 0.0;
                    for ( int i = lo; i < edgeIdx; ++i )
                        before += channelData->voltage.samples[ unsigned( i ) ];
                    before /= double( edgeIdx - lo );
                    double after = 0.0;
                    for ( int i = edgeIdx; i < hiAfter; ++i )
                        after += channelData->voltage.samples[ unsigned( i ) ];
                    after /= double( hiAfter - edgeIdx );
                    double localMax = channelData->voltage.samples[ unsigned( edgeIdx ) ];
                    double localMin = localMax;
                    for ( int i = edgeIdx; i < hiAfter; ++i ) {
                        double s = channelData->voltage.samples[ unsigned( i ) ];
                        localMax = std::max( localMax, s );
                        localMin = std::min( localMin, s );
                    }
                    if ( after > before ) { // rising edge
                        channelData->overshootRise = std::max( channelData->overshootRise, localMax - channelData->top );
                    } else if ( after < before ) { // falling edge
                        channelData->overshootFall = std::max( channelData->overshootFall, channelData->base - localMin );
                    }
                };
                edgeOvershoot( edge1, std::max( 1, edge2 - edge1 ) );
                if ( edge2 < right )
                    edgeOvershoot( edge2, std::max( 1, edge3 - edge2 ) );
            }
        }
