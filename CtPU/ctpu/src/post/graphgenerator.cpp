...
void GraphGenerator::generateGraphsTYvoltage( PPresult *result ) {
    ...
    for ( ChannelID channel = 0; channel < scope->voltage.size(); ++channel ) {
        ...
        const SampleValues &rawSampleValues = useVoltSamplesOf( channel, result, scope );

        // У4а — РАЗРЯДНОСТЬ (movingaverage.h): канал усредняется окном N.
        // Среднее сохраняет уровень, поэтому масштаб отсюда НЕ меняется —
        // прибавляются только эффективные биты (+0.5·log₂(N), закон Tektronix
        // HiRes) ценой полосы f_c ≈ 0.443·SR/N. Применяется ТОЛЬКО к трассе и
        // гистограмме; спектр считается от неусреднённых данных, иначе boxcar
        // исказил бы АЧХ выше f_c.
        //
        // У4б — ЭКРАННОЕ УВЕЛИЧЕНИЕ живёт отдельно и входит в displayGain().
        // Прежде обе функции сидели в одном параметре, и порог триггера
        // расходился с трассой ровно в √N раз (`docs/PROTOTYPE-QUEUE.md` №7).
        const unsigned resN = scope->voltage[ channel ].resolutionN;
        SampleValues averagedValues; // локальная копия только при активном усреднении
        unsigned zoomMargin = 0;
        bool averaged = false;
        if ( resN > 1 && rawSampleValues.samples.size() >= resN ) {
            averagedValues.interval = rawSampleValues.interval;
            averagedValues.samples = MovingAverage::apply( rawSampleValues.samples, resN );
            zoomMargin = MovingAverage::margin( resN );
            averaged = true;
        }
        const SampleValues &sampleValues = averaged ? averagedValues : rawSampleValues;
        ...
        if ( leftmostSample ) {                   // adjust position if triggered, else start from sample[0]
            ...
            // Центрированное усреднение съедает (N-1)/2 отсчётов слева:
            // триггерная позиция в усреднённом ряду сдвинута на zoomMargin.
            leftmostSample -= int( zoomMargin );
        }
        ...
        // У4б — единственное место пересчёта «деление ↔ величина» (scopesettings.h).
        const double gain = scope->displayGain( channel );
...
void GraphGenerator::generateGraphsXY( PPresult *result ) {
    ...
    result->vaChannelVoltage.resize( scope->voltage.size() );
    result->vaXYCurves.resize( DsoSettingsScope::maxXYCurves );

    // Delete all spectrum graphs
    for ( ChannelGraph &data : result->vaChannelSpectrum )
        data.clear();
    // В XY-режиме потоковые Y-T кривые не рисуются: их массив должен быть
    // пуст, иначе GlScope выведет остатки прошлого формата поверх XY.
    for ( ChannelGraph &data : result->vaChannelVoltage )
        data.clear();

    // [MOD] TZ §7.1 — draw up to maxXYCurves trajectories, each configured by
    // scope->xyCurves[i]. The legacy code iterated (CH1,CH2),(CH3,CH4) pairs;
    // the new scheme maps an arbitrary (xChannel, yChannel) pair per curve
    // onto the same grid. Color is taken from the y channel.
    for ( int curveIdx = 0; curveIdx < DsoSettingsScope::maxXYCurves; ++curveIdx ) {
        // [FIX] Своя ячейка на КРИВУЮ. См. PPresult::vaXYCurves: адресация по
        // yChannel склеивала кривые с общим Y и позволяла одной кривой
        // стереть другую.
        ChannelGraph &graphXY = result->vaXYCurves[ curveIdx ];
        graphXY.clear();

        const auto &cfg = scope->xyCurves[ curveIdx ];
        if ( !cfg.enabled )
            continue;
        if ( cfg.xChannel >= scope->voltage.size() || cfg.yChannel >= scope->voltage.size() )
            continue;

        const ChannelID xChannel = cfg.xChannel;
        const ChannelID yChannel = cfg.yChannel;
        ...
        // [MOD] TZ §6.1.1 — use physicalGain so the XY plot is in physical units
        // (e.g. °C vs °C) when CtPU is active on either channel.
        const double xGain = scope->physicalGain( xChannel );
        const double yGain = scope->physicalGain( yChannel );
        // [FIX] TZ §6.1.1/§7.1 — xOffset must be symmetric with yOffset: the
        // X channel's own per-channel offset (scope->voltage[xChannel].offset),
        // exactly like GlScope::updateXY() already does for the continuous
        // XYRecorder path. The previous formula, `(scope->trigger.position -
        // 0.5) * DIVS_TIME`, is TY-mode horizontal-sweep math left over from
        // before multi-channel XY existed: it has nothing to do with a
        // voltage/physical-unit X axis. Consequences of the old code: the
        // X-channel offset control in the UI silently did nothing in this
        // mode, while dragging the (unrelated) trigger-position slider
        // shifted the XY trace horizontally.
