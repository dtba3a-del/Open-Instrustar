...
DsoConfigDialog::DsoConfigDialog( DsoSettings *settings, QWidget *parent ) : QDialog( parent ), settings( settings ) {
    ...
    contentsWidget->setMinimumHeight( CONFIG_LIST_ITEMHEIGHT * 4 + 2 * ( contentsWidget->frameWidth() ) );
    ...
    ctpuMathPage = new DsoConfigCtpuMathPage( settings );
    pagesWidget->addWidget( ctpuMathPage );
...
void DsoConfigDialog::createIcons() {
    ...
    // [MOD] TZ §12.1 — CtPU / Math / XY configuration page.
    QListWidgetItem *ctpuMathButton = new QListWidgetItem( contentsWidget );
    ctpuMathButton->setIcon( QIcon( ":config/ctpu_math.png" ) );
    ctpuMathButton->setText( tr( "CtPU / Math" ) );
    if ( settings->scope.toolTipVisible )
        ctpuMathButton->setToolTip( tr( "Physical-unit conversion, math-stack virtual channels, and XY multi-curve configuration" ) );
...
void DsoConfigDialog::updateLiveData( std::shared_ptr< PPresult > data ) {
    if ( ctpuMathPage )
        ctpuMathPage->updateLiveData( data );
}
