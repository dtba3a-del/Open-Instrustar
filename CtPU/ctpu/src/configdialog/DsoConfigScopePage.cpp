...
DsoConfigScopePage::DsoConfigScopePage( DsoSettings *settings, QWidget *parent ) : QWidget( parent ), settings( settings ) {
    ...
    hasACmodificationCheckBox =
        new QCheckBox( tr( "Scope has hardware modification for AC coupling" ) );
    hasACmodificationCheckBox->setChecked( settings->scope.hasACmodification );
    hasACmodificationCheckBox->setToolTip(
        tr( "Only shown for models without built-in AC coupling. Check this ONLY if you have physically "
            "added the AC-coupling hardware modification to this specific board -- leaving it unchecked "
            "is always safe (inputs stay DC-coupled); checking it without the actual modification installed "
            "will silently mislabel DC-coupled readings as AC." ) );
