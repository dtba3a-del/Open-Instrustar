...
class ScopeDevice : public QObject {
  ...
  public:
    ...
    /// Дескриптор устройства как есть. Нужен, чтобы спросить систему, в
    /// каком классе драйвера находится этот `VID:PID`, ДО записи прошивки
    /// (`usb/driverclass.h`).
    inline const struct libusb_device_descriptor &getDescriptor() const { return descriptor; }
