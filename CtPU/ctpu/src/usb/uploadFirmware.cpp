...
bool UploadFirmware::startUpload( ScopeDevice *scopeDevice ) {
    ...
    // Класс драйвера проверяется ДО записи (распоряжение автора 2026-09-05).
    // Прежде заливка шла, не спрашивая, кто держит устройство: под вендорским
    // драйвером она проходит, прибор перечисляется как 1d50:608e, и за этот
    // идентификатор в системе не отвечает никто. Рабочее состояние пути A
    // при этом теряется, а путь B не приобретается — не работает НИ ОДИН.
    // Разбор — `ctpu/src/usb/driverclass.h`.
    const libusb_device_descriptor &d = scopeDevice->getDescriptor();
    const UsbDriverClass::Verdict verdict = UsbDriverClass::forDevice( d.idVendor, d.idProduct );
    if ( !UsbDriverClass::firmwareWriteAllowed( verdict ) ) {
        errorMessage = UsbDriverClass::explain( verdict );
        return false;
    }
