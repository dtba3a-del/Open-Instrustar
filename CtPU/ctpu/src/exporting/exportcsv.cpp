...
void ExporterCSV::fillHeaders( QTextStream &csvStream, const ExporterData &dto, const char *sep ) {
    ...
    for ( ChannelID channel = 0; channel < dto.getChannelsCount(); ++channel ) {
        if ( voltageData[ channel ] != nullptr ) {
            // [MOD] TZ §10.1.1 — use the channel's ctpuUnit instead of hard-coded "V".
            const QString unit = registry->settings->scope.voltage[ channel ].ctpuUnit.isEmpty()
                                     ? QStringLiteral( "V" )
                                     : registry->settings->scope.voltage[ channel ].ctpuUnit;
            csvStream << sep << "\"" << registry->settings->scope.voltage[ channel ].name << " / " << unit << "\"";
        }
    }
...
bool ExporterCSV::save() {
    ...
    // [MOD] Capture timestamp, requested in chat. ISO 8601 with milliseconds,
    // deliberately NOT QLocale-formatted (unlike the data values below) --
    // a timestamp needs to stay unambiguous and sortable regardless of the
    // user's regional settings. "#"-prefixed so spreadsheet/CSV tools that
    // skip comment lines handle it gracefully; tools that don't will just
    // see it as an extra one-column first row, which is harmless.
    csvStream << "# Captured: " << QDateTime::currentDateTime().toString( Qt::ISODateWithMs ) << "\n";
