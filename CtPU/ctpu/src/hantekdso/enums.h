...
/// \enum MathOp
/// \brief The four arithmetic operations supported by the math-stack (TZ §5.3).
enum class MathOp : unsigned { ADD, SUB, MUL, DIV };
extern Enum< Dso::MathOp, Dso::MathOp::ADD, Dso::MathOp::DIV > MathOpEnum;
const auto LastMathOp = MathOp::DIV;
...
enum class TriggerMode {
    ...
    ROLL,   ///< Free running without any trigger, slow timebases only
    /// \brief Поток без поиска уровня — задание 9 очереди прототипа.
    ///
    /// ROLL существовал, но предлагался только на развёртках медленнее
    /// 0,1 с/дел и назывался «Roll». Оператору нужен явный режим «отдавать
    /// как пришло» на ЛЮБОЙ развёртке: поиск уровня сдерживает поток и
    /// отображение сильно тормозит.
    ///
    /// Существо, названное автором: потокового захвата с триггером не бывает —
    /// либо поток непрерывен, либо отображение по условию. FREERUN есть
    /// первое, и он объявлен, а не спрятан.
    FREERUN
};          // <class T, T first, T last>
extern Enum< Dso::TriggerMode, Dso::TriggerMode::AUTO, Dso::TriggerMode::FREERUN > TriggerModeEnum;
