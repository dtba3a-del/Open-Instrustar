...
struct ControlSpecification {
    ...
    /// У9: analog front-end −3 dB bandwidth per gain step, Hz.
    /// The input amplifier trades bandwidth for gain (BW ≈ GBW/K), so the
    /// tract bandwidth depends on the V/div setting and must be shown to the
    /// operator. Datasheet-derived estimate, not measured per unit; empty =
    /// unknown (no indication).
    std::vector< double > analogBandwidth;
