...
static void initSpecifications( Dso::ControlSpecification &specification ) {
    ...
    // Встроенной AC-связи у MDSO НЕТ — она появляется только с аппаратной
    // доработкой (docs/HANTEK6022_AC_Modification.pdf). Прежний комментарий
    // «MDSO has AC coupling» противоречил самому значению и сбивал с толку
    // при поисках, почему AC недоступна. У ISDS205B, наоборот, связь
    // встроенная: modelISDS205b.cpp, hasACcoupling = true.
