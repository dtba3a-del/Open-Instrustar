...
static void initSpecifications( Dso::ControlSpecification &specification ) {
    ...
    // У9: mirror the target instrument's (ISDS205) per-step analog bandwidth
    // so the GUI indication can be exercised and click-verified in demo mode.
    // See modelISDS205b.cpp for the derivation (AD8065 datasheet Rev. E).
    specification.analogBandwidth = { 3.8e6, 3.8e6, 3.8e6, 12e6, 16e6, 16e6, 16e6, 16e6 };
