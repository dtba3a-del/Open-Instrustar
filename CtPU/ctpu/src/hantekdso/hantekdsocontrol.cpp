...
Dso::ErrorCode HantekDsoControl::setTriggerLevel( ChannelID channel, double level ) {
    if ( deviceNotConnected() )
        return Dso::ErrorCode::CONNECTION;
    // [MOD] TZ §5.6.2 — allow trigger on any unified channel index (real +
    // math-stack). The old guard `channel > specification->channels` only
    // permitted the legacy single MATH slot (index = spec->channels).
    if ( channel >= specification->channels + DsoSettingsScope::maxMathChannels )
        return Dso::ErrorCode::PARAMETER;
    if ( verboseLevel > 2 )
...
void HantekDsoControl::applySettings( DsoSettingsScope *dsoSettingsScope ) {
    ...
    // [MOD] TZ §5.6.2 — iterate over ALL unified channel slots (real + math-stack)
    // so setTriggerLevel/setGain/setProbe are applied to M1..M4 too. The old
    // loop `channel <= specification->channels` only reached the legacy MATH slot.
    const ChannelID totalChannels = specification->channels + DsoSettingsScope::maxMathChannels;
    for ( ChannelID channel = 0; channel < totalChannels; ++channel ) {
    ...
    mathStack = std::unique_ptr< MathStack >( new MathStack( scope, specification->channels ) );
...
void HantekDsoControl::convertRawDataToSamples() {
    ...
    result.tag = raw.tag;
    result.captureTimestampMs = QDateTime::currentMSecsSinceEpoch();
    result.samplerate = raw.samplerate / raw.oversampling;
    // Prepare result buffers
    // [MOD] TZ §5.6.1 — pre-allocate all channel slots (real + math) so that
    // MathStack::calculate() and downstream consumers can index result.data
    // without triggering another resize mid-pipeline.
    result.data.resize( specification->channels + DsoSettingsScope::maxMathChannels ); // CH0, CH1, M1..M4
    for ( ChannelID channelCounter = 0; channelCounter < result.data.size(); ++channelCounter )
...
void HantekDsoControl::stateMachine() {
    ...
    if ( samplingStarted && raw.valid && ( raw.tag != lastTag || raw.freeRun || refreshNeeded() ) ) {
        ...
        // [NEW] TZ §3.4 — Apply CtPU to physical channels (CH1, CH2, ...).
        // Math channels get their own CtPU inside MathStack::calculate().
        // Hot-path safe: inline arithmetic, no allocations, no virtual calls.
        if ( scope ) {
            const unsigned nPhys = specification->channels;
            const unsigned nTotal = unsigned( result.data.size() );
            // Keep the per-channel physicalUnits vector in sync with the data vector.
            if ( result.physicalUnits.size() < nTotal )
                result.physicalUnits.resize( nTotal, QStringLiteral( "V" ) );
            for ( unsigned ch = 0; ch < nPhys && ch < nTotal; ++ch ) {
                const auto &v = scope->voltage[ ch ];
                if ( v.ctpuMode != CtPU::Mode::OFF ) {
                    CtPU::applyToVector( result.data[ ch ], v.ctpuK, v.ctpuB );
                }
                result.physicalUnits[ ch ] = v.ctpuUnit;
            }
        }

        // [MOD] TZ §5 — Configurable math stack (2-4 channels) replaces the
        // monolithic MathChannel. The legacy mathChannel wrapper is kept for
        // backward compatibility but is no longer called from the hot path.
        // mathChannel->calculate( result );  // legacy, single MATH slot
        if ( mathStack )
