...
class DsoSettings {
  ...
  public:
    ...
    /// \name Выбор камеры — общие настройки программы, а не файл прибора
    ///
    /// Камера принадлежит машине, а не осциллографу, и не должна пропадать при
    /// смене прибора, в демо-режиме или при выключенном `alwaysSave`. Пишется
    /// сразу по нажатию «Применить»; `save()` этих ключей не трогает, чтобы у
    /// одной величины остался один хозяин.
    ///@{
    void loadCameraChoice();
    void saveCameraChoice();
    ///@}
